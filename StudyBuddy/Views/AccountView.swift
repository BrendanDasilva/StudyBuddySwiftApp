...
struct AccountView: View {
    @State private var isLoggedOut = false  // Flag to check if the user is logged out
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(isPresented: $isLoggedOut) {
                LoginView(isLoggedIn: .constant(false))  // Navigate back to LoginView after logout
            }
        }
    }

    private func logout() {
        // Clear UserDefaults to log out
        UserDefaults.standard.set(false, forKey: "isLoggedIn")
        UserDefaults.standard.removeObject(forKey: "userEmail")
        
        // Set the flag to trigger the navigation to LoginView
        isLoggedOut = true
    }
}
